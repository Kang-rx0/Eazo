...
# V3 P0 收紧："血压/血糖"附近的数字要"像读数"才删，三个条件取并——
#   ① 读数形态 140/90；② 间隔里带取值语境词（控制在/不超过…）；③ 血糖紧邻（≤2字）小数。
#   数字后紧跟 分钟/小时/点/年/月/日/版/次/步 的是时间时长版次、不是读数，一律排除
#   （曾误伤："量完血压再走10分钟"→"量完分钟"、"高血压人群晚上8点后"、"睡前测个血压 22:30"、
#   "成人高血压食养指南2023版"）；"点几"不算排除——"血糖7点几"是口语读数。
#   数字后的 (?![\d./]) 防止回溯把"140分钟"缩成"14"绕过排除、或截断小数留残尾。
#   第一分支（带 mmHg/mmol 单位）不动：有单位的永远删。
_MED_NUM_RE = re.compile(
    r"[约大概≈~]*\d+(?:\.\d+)?(?:\s*/\s*\d+(?:\.\d+)?)?\s*(?:mmHg|毫米汞柱|mmol/?L?)"
    r"|(?:血压|血糖)[^，。；！？()（）《》]{0,6}?\d{2,3}\s*/\s*\d{2,3}(?:\.\d+)?"
    r"(?![\d./])(?!\s*(?:分钟|小时|点(?!几)|年|月|日|版|次|步))"
    r"|(?:血压|血糖)[^，。；！？()（）《》]{0,6}?(?:控制在|不超过|超过|低于|高于|降到|达到|目标|保持在)"
    r"[^，。；！？()（）《》]{0,3}?\d+(?:[./]\d+(?:\.\d+)?)*"
    r"(?![\d./])(?!\s*(?:分钟|小时|点(?!几)|年|月|日|版|次|步))"
    r"|血糖[^，。；！？()（）《》]{0,2}?\d+\.\d+"
    r"(?![\d./])(?!\s*(?:分钟|小时|点(?!几)|年|月|日|版|次|步))"
